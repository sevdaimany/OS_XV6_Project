...
growproc(int n)
{
  ...
  // we should update sz in all threads due to existance of clone system call
  struct proc *p;
...
fork(void)
{
  ...
  // there is only one thread for the child because in fork, fork creates a new pgdir for the cild
  np->threads = 1;
  np->parent = curproc;
  *np->tf = *curproc->tf;
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        p->killed = 0;
        p->state = UNUSED;
        // reset stack top and pgdir if it is parent
        p->stackTop = 1;
        p->pgdir = 0;
        p->threads = -1;
        release(&ptable.lock);
...
// Remmmber stack grows downwards thus the stackTop will be in the address given by parent
